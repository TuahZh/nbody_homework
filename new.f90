...
!real(8) time,x_diff,y_diff,z_diff,dist,dt,time_max
!real(8) pos(10,3),vel(10,3),force(10,3)
!real(8) mass(10),force_3(3)     !!变量声明位置固定？
real(8) time,x_diff,y_diff,z_diff,dist,dt,time_max,loo,er,cj,dt_max,mass_s
real(8) pos(99,3),vel(99,3),r(99,6),forces(99,3),pos_r(99,3),vel_r(99,3),pos_bc(3),vel_bc(3),pos_s(3)
real(8) force(5,99,3),f(14,99,6)
!real,external :: fof
real(8) mass(99),force_3(3)     
...
vel(2,3)=0.0/V0
vel(3,1)=0.0/V0
vel(3,2)=vel(2,2)+1.02D3/V0
vel(3,3)=0.0/V0
print *, vel(2,2), V0
!vel(4,1)=0.0
!vel(4,2)=-133.0/140*vel(2,2)
!vel(4,3)=0.0
er=0.01
!!!!!calc barycentre!!!!!
pos_bc=0
...
do k=1,3
do i=1,num
!pos_bc(k)=mass(i)*pos_bc(k)+pos(i,k) !???
!vel_bc(k)=mass(i)*vel_bc(k)+vel(i,k)
pos_bc(k)=pos_bc(k)+mass(i)*pos(i,k) !???
vel_bc(k)=vel_bc(k)+mass(i)*vel(i,k)
end do
mass_s=0
do i=1,num
...
!        &pos(i,3)-pos_s(3)
    write(10,'(i5,1x,g18.8,1x,g18.8,1x,g18.8,1x,g18.10)') i, pos(i,1)-pos_s(1),pos(i,2)-pos_s(2),&
        &pos(i,3)-pos_s(3), time
    end do
...
f=0
  ...
  !  print*,f(1,1,1),f(1,1,4),pos_r(1,1)
	...
	loo=abs(41.0/810*dt*(f(1,1,1)+f(11,1,1)-f(12,1,1)-f(13,1,1)))
	if (loo > er ) then
	dt=dt/2.0
    print*,'loser!'
	time=time-dt
...
!   	write(10,'(f18.8,1x,f18.8,1x,f18.8,1x,f18.8,1x,f18.8,1x,f18.8,1x,f18.8,1x,f18.8,1x,f18.8,1x,f8.5)')&
   ! pos(19,1),pos(19,2),pos(19,3),pos(89,1),pos(89,2),pos(89,3),pos(29,1),pos(29,2),pos(29,3),time
    ...
    do i=1,3
!    pos_s(i)=pos_bc(i)+(dt+time)*vel_bc(i)
	pos_bc(i)=pos_bc(i)+dt*vel_bc(i)
    end do
!    write(10,'(f18.8,1x,f18.8,1x,f18.8,1x,f18.10)') pos(num,1)-pos_s(1),pos(num,2)-pos_s(2),&
!        &pos(num,3)-pos_s(3),time
!print*,pos(1,2)
	end if
time=time+dt
end do
...
implicit none
integer num,i,k,thread_num,thread_count,rang  !!rang for k(rang)
integer OMP_GET_NUM_THREADS,OMP_GET_THREAD_NUM
real(8) x_diff,y_diff,z_diff,dist
real(8) pos(99,3),vel(99,3),forces(99,3)
real(8) force(5,99,3),f(14,99,6)
real(8) mass(99),force_3(3)     
...
!real(8),dimension(14,10,6) fof  !10 for number of stars,14 for rkf range
